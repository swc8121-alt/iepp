...
We present \iepp{}, the \emph{Individual Entity Proof Protocol}. Earlier project documents expanded IEPP as ``Intrinsic Entropy Proof of Presence''; the current name reflects the narrower protocol claim. Rather than equating identity with structural uniqueness, \iepp{} asks whether authenticated evidence advances an accepted trajectory.
...
\section{Reader's Guide to IEPP}
\subsection{A minimal example}
Suppose one agent finishes work yesterday and is copied into two processes today. Both copies may have the same name, model, memory, and signing key, so ordinary login and signature verification can accept both. IEPP does not ask which process is the metaphysically ``real original.'' It asks a narrower, operational question: relative to a verifier's acceptance record, which candidate validly advances the previously accepted state?

At each step, the verifier issues a challenge that can be used only once. A candidate binds that challenge, the previously accepted state, the next sequence number, and declared current-execution evidence into a signed transition. The registry checks the evidence and atomically records at most one successor. A later copy may still produce a valid signature, but its challenge has been consumed or its predecessor is stale, so it is not accepted as the canonical successor.
...
\subsection{Notation and fixed labels}
The strings \texttt{IEPP-Entropy-v1} and \texttt{IEPP-State-vNext-1} below are fixed ASCII protocol labels, not mathematical variables. They provide domain separation so hashes created for different purposes or protocol versions cannot be silently interchanged. The latter label is retained for compatibility with the current public reference implementation.
...
\begin{equation}
  EC_t=\Hash(\texttt{IEPP-Entropy-v1}\parallel E_t).
\end{equation}
...
For a fresh challenge $(cid_t,nonce_t,expiry_t)$ and monotonic counter $t$, let $D=\texttt{IEPP-State-vNext-1}$ and let $R_t$ and $A_t$ denote the optional runtime and attestation commitments. The prover computes
\begin{align}
S_t=\Hash(&D\parallel id\parallel domain\parallel t\parallel S_{t-1}\nonumber\\
