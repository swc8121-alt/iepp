...
\end{quote}
All reported results remain L1 evidence. The A3 evaluation additionally includes controlled VirtualBox/two-VM execution, but no hardware-backed attestation.
...
\subsection{Controlled A3 VM restore and two-worker races}
A controlled A3 run used an actual VirtualBox snapshot at accepted state P1. After restoring that VM state, a fresh signed P1$\rightarrow$P2 transition was accepted. The registry was then left at P2 while the VM was again returned to the earlier P1 state; a newly signed counter-2 candidate from that stale state was rejected as \texttt{ROLLBACK\_OR\_LOSING\_FORK}, and the canonical P2 head remained unchanged. The public evidence bundle is identified by SHA-256 \texttt{0e35e44973c0476fd8423d056e5d7c632a92eba4a951bf3001ec2affe77fc453}. This is operator-collected VirtualBox evidence, not hypervisor attestation.

A separate two-VM race harness evaluated 220 independent trials on source revision \texttt{f9848ae8cd7b4db6c90634b565205b3df9da8a39}: 110 fresh-challenge races and 110 shared-challenge races. Every trial produced exactly one canonical acceptance and one rejection, with zero double accepts. The fresh loser was rejected as \texttt{ROLLBACK\_OR\_LOSING\_FORK} in 110/110 cases, and the shared-challenge loser as \texttt{CHALLENGE\_USED} in 110/110 cases. Independent rechecking verified all 440 Ed25519 signatures, evidence identifiers, state-transition hashes, and final heads. Sixty-six of 220 trials had overlapping observed server-processing intervals; this overlap indicates concurrent server activity or waiting, not simultaneous execution of the compare-and-swap critical section. The race artifact SHA-256 is \texttt{66b4f0d759d1b23cac6dbf25a502b08b912863c1602e0d5a3a07ea5e04d92a02}. The race batch did not restore a VM on every trial.
